...
- [ ] **命名管道 ACL 限 SYSTEM**:现为默认 ACL(创建者 + 管理员 + SYSTEM 可访问),需收紧到只允许 SYSTEM(防本地非特权进程冒充 CP 调认证)。代码位置 `service.py` 的 `CreateNamedPipe`。
- [ ] **失败兜底 / 锁定**:连续失败次数限制、设备忙 / 摄像头不可用时的优雅降级,避免锁屏卡死。
- [ ] **日志完善**:服务侧已落 `service.log`,需补结构化与脱敏审计。
