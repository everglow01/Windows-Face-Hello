...
| **5-4 加固:命名管道 ACL 限 SYSTEM、失败兜底 / 锁定、日志完善** | ⏳ 做了一半 |
...
被动反欺骗已启用(设置页可关):在识别帧上跑一次 MiniFASNet 推理,翻拍/假体在比对前就拒掉,补上主动活体挡不住的「录制视频回放」。两个坑:① MiniFASNet 对裁剪框极敏感,必须用配套 RetinaFace 的框(InsightFace 的紧框会判错);② 输入是 0~255 的 BGR(官方 ToTensor 把 `/255` 注释掉了)。
...
- [x] **被动反欺骗**:Silent-Face MiniFASNet(`2.7_80x80`)+ RetinaFace 裁剪,识别帧上一次推理判翻拍;默认开、可关、模型缺失 fail-open。
- [ ] **可选增强**:GPU 推理、pytest、进一步瘦身。
